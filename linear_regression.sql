...
//
CREATE OR REPLACE VIEW LINEAR_REGR
AS
WITH SCOPED AS (
    SELECT SEGMENT_ID
      --,PRODUCT_LINE_ID
  ...
  ,FULL_MAP AS (
    SELECT *
    FROM (SELECT 0 SEGMENT_ID, 0 DAYSHIFT_ID, CURRENT_DATE()-1 DATA_DATE, CURRENT_DATE()-7 SCOPE_BEGIN
          UNION ALL
          SELECT SEGMENT_ID, DAYSHIFT_ID, DATA_DATE, DATEADD(DAY, -6, DATA_DATE) SCOPE_BEGIN 
		  FROM SCOPED 
  ...
  ,SHIFTDAY_AVG AS (
    SELECT DAYSHIFT_ID
      ...
      ,COUNT(SPEND) CNT
      ,SUM(SPEND) SPEND_SUM
      ,SUM(SPEND)/COUNT(SPEND) SPEND 
    FROM SCOPED 
    WHERE SEGMENT_ID != 0
    GROUP BY DAYSHIFT_ID
      --//,SEGMENT_ID
      --,PRODUCT_LINE_ID
      ,NETWORK_ID
      ,ACCOUNT_ID
  ...
  ,SLIDING_AVG AS (
    ...
    ON B.DATA_DATE BETWEEN A.SCOPE_BEGIN AND A.DATA_DATE
      AND A.SEGMENT_ID = CASE B.SEGMENT_ID WHEN 0 THEN 0 ELSE 1 END
  ...
  ,SHIFTDAY_RATIO AS (
    SELECT A.SEGMENT_ID
        ...
        ,A.DATA_DATE
        ,A.SPEND
        ,B.SPEND/NULLIF(A.SPEND,0) SHIFTDAY_FACTOR
    FROM SLIDING_AVG A
    LEFT JOIN SHIFTDAY_AVG B
    ON CASE A.DAYSHIFT_ID WHEN 0 THEN -7 ELSE A.DAYSHIFT_ID END = B.DAYSHIFT_ID
  ...
  ,LINEAR_REGR AS (
    SELECT SEGMENT_ID
      ...
      ,NETWORK_ID
      ,ACCOUNT_ID
      ,REGR_SLOPE(SPEND, DAYSHIFT_ID) SLOPE
      ,REGR_INTERCEPT(SPEND, DAYSHIFT_ID) INTERCEPT
    FROM SLIDING_AVG
    GROUP BY SEGMENT_ID
      --,PRODUCT_LINE_ID
      ,NETWORK_ID
  ...
  ,DIFFERENCE AS (
    SELECT A.DATA_DATE
        ...
        ,A.NETWORK_ID
        ,A.ACCOUNT_ID
        ,B.SLOPE * A.DAYSHIFT_ID + B.INTERCEPT PREDICTION_SPEND
        ,A.SPEND ACTUAL_SPEND
        --,ROUND(100*((B.SLOPE * A.DAYSHIFT_ID + B.INTERCEPT - A.SPEND)/NULLIF(A.SPEND,0)),2) DIFF 
    FROM SHIFTDAY_RATIO A 
    LEFT JOIN LINEAR_REGR B 
    ON CASE A.SEGMENT_ID WHEN 0 THEN 1 ELSE A.SEGMENT_ID END = B.SEGMENT_ID
    --AND A.PRODUCT_LINE_ID = B.PRODUCT_LINE_ID
    AND A.NETWORK_ID = B.NETWORK_ID
    AND A.ACCOUNT_ID = B.ACCOUNT_ID
...
SELECT *
FROM BI_TEST._CONTROL_LOGIC.LINEAR_REGR
