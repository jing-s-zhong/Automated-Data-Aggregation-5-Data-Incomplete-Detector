...
--
-- Spend tracking rows populator SP
--
-- CALL ALERTS.BUYSIDE_ACCOUNT_DATA_TRACK_ROWS_POPULATOR (300);
CREATE OR REPLACE PROCEDURE BUYSIDE_ACCOUNT_DATA_TRACK_ROWS_POPULATOR (
...
AS
$$
var AmountThreshold = AMOUNT_THRESHOLD ? AMOUNT_THRESHOLD : 300.;
var snowRet = '', snowSql = `
-- under github https://github.com/Openmail/BusinessIntelligence/tree/master/S1-Data-Incomplete-Detector/scripts/buyside_account_data_track_rows_populator.sql
...
USING (
    ...
    ,REVISED_CAST AS (
      SELECT A.DATA_DATE
        ...
        ,CASE WHEN B.ACC_RATIO BETWEEN .85 AND 1.15 THEN A.SPEND_AVG7 ELSE A.SPEND_AVG3 END -- Classfier-1: account runnng stable or trending up/down
            * COALESCE(CASE WHEN (B.ACC_RATIO BETWEEN .95 AND 1.05) AND (B.FINE_RATIO BETWEEN .7 AND 1.3) THEN B.FINE_RATIO ELSE B.DAY_RATIO END, 1) SPEND_FORECAST -- Classfier-2: account just weekday senstive or random change in large range
        ,A.SPEND_ACTUAL
      FROM LINEAR_REGR A
  ...
  SELECT DATA_DATE
      ...
      ,CASE WHEN (SPEND_AVG7 > :1) AND COALESCE(SPEND_ACTUAL, 0) = 0 THEN 1 
        ELSE 0 END::BOOLEAN ALERT_TRIGGER
      ,True ALERT_ALLOWED
  FROM REVISED_CAST
  WHERE (SPEND_AVG7 > 0 AND DATA_DATE = CURRENT_DATE()-1)
  ) S
...
WHEN MATCHED THEN UPDATE SET
    ...
    ,ALERT_STATUS = S.ALERT_TRIGGER AND D.ALERT_ALLOWED AND COALESCE(D.ALERT_STATUS, S.SPEND_FOUND_TIME IS NOT NULL) OR (D.ALERT_STATUS AND S.SPEND_FOUND_TIME IS NULL) 
