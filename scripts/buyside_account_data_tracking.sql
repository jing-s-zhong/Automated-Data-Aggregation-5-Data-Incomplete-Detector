...
USING (
  ...
  SELECT DATA_DATE
      ...
      ,CASE WHEN (SPEND_AVG3 > 300.) AND (SPEND_AVG7 - SPEND_AVG3 < SPEND_AVG7 / 3) 
					AND (SPEND_FORECAST - COALESCE(SPEND_ACTUAL,0) > 300.) 
					AND (SPEND_FORECAST - COALESCE(SPEND_ACTUAL,0))/NULLIF(SPEND_FORECAST,0) > .5 
				THEN 1 
			ELSE 0 END::BOOLEAN ALERT_STATUS
      ,True ALERT_ALLOWED
  FROM REVISED_CAST
